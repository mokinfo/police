...
<script>
  ...
  <div class="content-wrapper">
                            ...
                            <table class="table  table-striped table-bordred table-hover" id="jaktab">
                              ...
                              <tbody><?php 
                                  while($data = $sql->fetch_array()){
                                    ...
                                    $idh = $data['idh'];
                                    $sortie_check = $conn->query("SELECT * FROM sortie_hos WHERE ids = '$idh'");
                                    $sortie_exists = $sortie_check->num_rows > 0;
                                    ...
                                    echo '<tr>
                                        ...
                                        <td><a href="Affiche_Hospitalisation?id=' . $data['idh'] .' " style=\"text-align:center;\"><i class="fas fa-eye fa-2x"></i></a></td>';
                                    if ($sortie_exists) {
                                        echo '<td><a href="Ticket_Sortie?id=' . $data['idh'] . '" style="text-align:center;" target="_blank"><i class="fa fa-print text-warning"></i></a></td>';
                                    } else {
                                        echo '<td></td>';
                                    }
    ...
    <div class="card-header bg-light">
        <?php 
            $conn = new mysqli('localhost', 'root', '', 'spn');
            $sql = $conn->query("SELECT hospitalisation.idh ,hospitalisation.idp ,hospitalisation.med_trai, hospitalisation.date_hos, hospitalisation.chambre, hospitalisation.service, hospitalisation.etat, patient.nom, patient.civi, patient.sex, patient.age, patient.catp, facture.datef, hospitalisation.date_hos FROM patient, hospitalisation, facture where patient.idp = hospitalisation.idp and hospitalisation.etat = 'HOS' and hospitalisation.idp = facture.idp and DATE(facture.datef) = hospitalisation.date_hos AND facture.etat = 'OK' group by hospitalisation.idp ORDER BY hospitalisation.idh DESC");
            $row_cnt = $sql->num_rows;
        ?>
        <h5 class="card-title">Nombres des patients hospitalisés : <b><?php echo($row_cnt); ?></b></h5>
    </div>
                ...
                <table class="table  table-striped table-bordred table-hover" id="jaktab">
                    ...
                    <tbody>
                        ...
                        while($data = $sql->fetch_array()){
                            ...
                            $catp = $data['catp'];
                            $is_special_catp = in_array($catp, ['pol', 'fpol', 'cas']);
                            ...
                            echo '<tr>
                                ...
                                <td>'.$data['chambre'].'</td>
                                <td>'.$etat.'</td>';
                            if ($sortie_exists || $is_special_catp) {
                                echo '<td><a href="Ticket_Sortie?id=' . $data['idh'] . '" style="text-align:center;" target="_blank"><i class="fa fa-print text-warning"></i></a></td><td><a href="Affiche_Hospitalisation?id=' . $data['idh'] .' " style=\"text-align:center;\"><i class="fas fa-eye fa-2x"></i></a></td>';
                            } else {
                                echo '<td></td>';
                            }
                            echo '</tr>';
