      ...
      <div class="container-fluid">
        <div class="row mb-12">
          ...
          <div class="col-sm-12">
            ...
            if ($role == 8) {
              $datess = date("d/m/Y");
              $pieces = $DB->query("SELECT entre.ident, entre.num, entre.motif, entre.num_medecin, entre.nom_medecin, entre.heura, entre.heurf, entre.statut, entre.codepatient, patient.idp, patient.nom, patient.sex, patient.daten FROM patient, entre WHERE entre.codepatient = patient.idp AND entre.datent = '$datess' AND (entre.statut = 'A' OR entre.statut = 'C' OR entre.statut = 'B' OR entre.statut = 'P') ORDER BY entre.statut");
              ?>
              <div id="zonereq">
                <?php echo count($pieces); ?> patient(s) en attente
              </div>
                ...
                <tbody>
                  <?php foreach ($pieces as $piece): ?>
                    ...
                    $affectation = [
                      ...
                      "HEM" => "Hématologie",
                      "URG" => "Urgence"
                    ][$piece->num_medecin] ?? "Inconnu";
                    ?>

                    <tr <?= $piece->idp & 1 ? 'bgcolor="#fff5e6"' : ''; ?>>
                      <td><i class="fa <?= $piece->sex == "Féminin" ? 'fa-female text-danger' : ($piece->sex == "Masculin" ? 'fa-male text-primary' : 'fa-user text-light'); ?> align-items-center"></i></td>
                      <td><?= htmlspecialchars($piece->num) ?></td>
                      <td><?= htmlspecialchars($piece->nom) ?></td>
                      <td><?= htmlspecialchars($piece->daten) ?></td>
                      <td><i class="fa <?= $piece->statut == "A" ? 'fa-hourglass-half text-warning' : ($piece->statut == "F" ? 'fa-check-circle text-success' : 'fa-arrow-circle-right text-primary'); ?> align-items-center"></i> <?= $piece->statut == "A" ? 'Non payer' : ($piece->statut == "F" ? 'Terminer' : 'En consultation') ?></td>
                      <td><?= htmlspecialchars($piece->heura) ?></td>
                      <td><?= htmlspecialchars($piece->heurf) ?></td>
                      <td><?= htmlspecialchars($affectation) ?></td>
                      <td><?= htmlspecialchars($piece->motif) ?></td>
                      <td>
                        <a href="Ticket?id=<?= $piece->ident ?>" target="_blank"><i class="fa fa-print"></i></a> &nbsp;&nbsp; | &nbsp;&nbsp;
                        <a href="Consultation?id=<?= $piece->ident ?>"><i class="fa fa-eye text-warning align-items-center"></i></a> &nbsp;&nbsp; | &nbsp;&nbsp;
                        <a href="Dossier?id=<?= $piece->idp ?>"><i class="fa fa-folder-open text-success align-items-center"></i></a>
                      </td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
                ...
                <tbody>
                  <?php foreach ($pieces as $piece): ?>
                    ...
                    <tr <?= $piece->idp & 1 ? 'bgcolor="#fff5e6"' : ''; ?>>
                      ...
                      <td><?= htmlspecialchars($piece->nom) ?></td>
                      <td><?= htmlspecialchars($piece->daten) ?></td>
                      <td><i class="fa <?= $piece->statut == "A" ? 'fa-hourglass-half text-warning' : ($piece->statut == "F" ? 'fa-check-circle text-success' : 'fa-arrow-circle-right text-primary'); ?> align-items-center"></i> <?= $piece->statut == "A" ? 'Non payer' : ($piece->statut == "F" ? 'Terminer' : 'Payer') ?></td>
                      <td><?= htmlspecialchars($piece->heura) ?></td>
                      <td><?= htmlspecialchars($piece->heurf) ?></td>
                      <td><?= htmlspecialchars($affectation) ?></td>
                      <td><?= htmlspecialchars($piece->motif) ?></td>
                      <td>
                        <a href="Ticket?id=<?= $piece->ident ?>" target="_blank"><i class="fa fa-print"></i></a><?php /* ?>
                        <?php if ($piece->statut == "B" or $piece->statut == "C" or $piece->statut == "P") {
                          # code...?> &nbsp;&nbsp; | &nbsp;&nbsp; <a href="Consultation?id=<?= $piece->ident ?>"><i class="fa fa-eye text-warning align-items-center"></i></a> &nbsp;&nbsp; | &nbsp;&nbsp;
                        <a href="Dossier?id=<?= $piece->idp ?>"><i class="fa fa-folder-open text-success align-items-center"></i></a><?php  ?>
                          <?php 
                        } */?>
                      </td>
                    </tr>
            ...
            if ($role == 3) {
              ...
              ?>
              <div id="zonereq">
                <?= count($pieces); ?> patient(s) en attente
              </div>
              <table class="table table-striped table-bordered table-hover" id="jaktab" style="width:100%">
                <thead>
                  <tr>
                ...
                <tbody>
                  <?php foreach ($pieces as $piece): ?>
                    ...
                    <tr <?= $piece->idp & 1 ? 'bgcolor="#fff5e6"' : ''; ?>>
                      ...
                      <td><?= htmlspecialchars($piece->nom) ?></td>
                      <td><?= htmlspecialchars($piece->daten) ?></td>
                      <td><i class="fa <?= $piece->statut == "B" ? 'fa-hourglass-half text-warning' : ($piece->statut == "F" ? 'fa-check-circle text-success' : 'fa-arrow-circle-right text-primary'); ?> align-items-center"></i> <?= $piece->statut == "B" ? 'En attente' : ($piece->statut == "F" ? 'Terminer' : 'En attente') ?></td>
                      <td><?= htmlspecialchars($piece->heura) ?></td>
                      <td><?= htmlspecialchars($piece->heurf) ?></td>
                      <td><?= htmlspecialchars($affectation) ?></td>
                      <td><?= htmlspecialchars($piece->motif) ?></td>
                  ...
                  <form action="nentree.php" method="post">
                    ...
                    ?>
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" name="paix" required>
