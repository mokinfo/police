...
$DB = new DB();
  ...
  $codecnss = 0;
  //var_dump($_POST);
    $sql = "INSERT INTO factures (codf, idp, datef, type_paix, mtt, nature) VALUES ('$codf', '$codecnss', '$datefacture', '$typepaix', '$mtt', '$nature')";
    $mysqli = new mysqli('localhost','root','','spn');
    $mysqli->query($sql);
    
    if (isset($mysqli)) {
      $conq = new mysqli('localhost','root','','spn');
      $queryq = "SELECT patient.idp, patient.nom, patient.numcnss, facture.idf, facture.type_paix, facture.desg,facture.mt,facture.mt_cnss,facture.datef FROM patient, facture WHERE facture.datef BETWEEN '$datedebut' AND '$datefin' and mt_cnss > 0 AND facture.idp = patient.idp";
                      ...
                      <div class="col-xl-9">
                        <?php 
                          ...
                          while($query_r = mysqli_fetch_assoc($query_runxf)){
                            $sexam = $query_r["id"] + 1;
                        ?>
                        <input type="hidden" name="idf" value="<?php echo $sexam; echo date("Y"); ?>">
                        <?php 
                        $dated = $_POST['dated'];
                        $datef = $_POST['datef']; ?>
                        <input type="hidden" name="datedebut" value="<?php echo $dated; ?>">
                        <input type="hidden" name="datefin" value="<?php echo $datef; ?>">
                          ...
                          <tbody>
                            <?php 
                              ...
                              if(isset($dated) && isset($datef))
                              {
                                ...
                                $to_date = $datef;
                                $idps = $idp;
                                $queryg = "SELECT * FROM facture WHERE datef BETWEEN '$from_date' AND '$to_date' and mt_cnss > 0";
